...
public class DownloadService extends Service {
	...
	public static Map<String, DownloadThread> threadcache = new HashMap<String, DownloadThread>();
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Notification notification = new Notification(R.drawable.ic_launcher,
				getString(R.string.app_name) + " 正在努力升级中",
		...
		contentView1.setTextViewText(R.id.n_title, getString(R.string.app_name)
				+ " 正在努力升级中");
		contentView1.setTextViewText(R.id.n_text, 0 + "% ");
		contentView1.setProgressBar(R.id.n_progress, 100, 0, false);
		notification.contentView = contentView1;

		final DownloadThread thread = new DownloadThread(notification, apkurl,
				notifyflag++);
		...
		public void handleMessage(Message msg) {
			...
			if (!Thread.currentThread().isInterrupted()) {
				...
				case 2:
					...
					if (data.changer) {
						UpgradeManager.getInstance().recordUpgradeState(
								UpgradeManager.UPGRADE_STATE_DONE);
						openfile(data.Uri);
					} else {
						Log.i("avi", "取消下载");
					...
					Intent cancel = new Intent()
							.setAction("android.basic.button.click.cancel");
				...
				case -1:
					...
					Toast.makeText(getApplication(), data.filename + "下载异常终止",
							1).show();
					Intent cancel2 = new Intent()
							.setAction("android.basic.button.click.cancel");
					cancel2.putExtra("url", data.url);
					sendBroadcast(cancel2);
					UpgradeManager.getInstance().recordUpgradeState(
							UpgradeManager.UPGRADE_STATE_FAIL_DOWNLOAD);
					break;
				}
			}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("android.basic.notification.click.cancel")) {
				...
				if (threadcache.containsKey(url)) {
					DownloadThread onethread = threadcache.get(url);
	...
	public class DownloadThread extends Thread {
		...
		public DownloadThread(Notification notification, String apkurl,
				int notifyflag) {
			super();
							...
							if (getSuspend()) {
								try {
									...
									Intent pause = new Intent()
											.setAction("android.basic.button.click.pause");
