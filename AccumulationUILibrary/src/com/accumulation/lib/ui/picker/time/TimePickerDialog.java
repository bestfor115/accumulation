...
package com.accumulation.lib.ui.picker.time;
...
import com.accumulation.lib.ui.TimeFormateTool;
import com.accumulation.lib.ui.picker.HapticFeedbackController;
import com.accumulation.lib.ui.picker.TypefaceHelper;
import com.accumulation.lib.ui.picker.Utils;
import com.accumulation.lib.ui.picker.time.RadialPickerLayout.OnValueSelectedListener;
...
//	private int mInitialMinute;
	...
	public interface OnTimeSetListener {
		 ...
		 */
		void onTimeSet(RadialPickerLayout view, Date date);

		void onDateClick(RadialPickerLayout mTimePicker, Date date);
	...
	public static TimePickerDialog newInstance(OnTimeSetListener callback,
			Date date, boolean is24HourMode) {
		TimePickerDialog ret = new TimePickerDialog();
		Calendar now = Calendar.getInstance();
		now.setTime(date);
		 ret.initialize(callback, now, is24HourMode);
		return ret;
	}

	public void initialize(OnTimeSetListener callback,Calendar c, boolean is24HourMode) {
		mCallback = callback;

		mCalendar.set(Calendar.YEAR, c.get(Calendar.YEAR));
		mCalendar.set(Calendar.MONTH, c.get(Calendar.MONTH));
		mCalendar.set(Calendar.DAY_OF_MONTH, c.get(Calendar.DAY_OF_MONTH));
...
//			mInitialMinute = savedInstanceState.getInt(KEY_MINUTE);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mHourPickerDescription = res
				.getString(R.string.mdtp_hour_picker_description);
		mSelectHours = res.getString(R.string.mdtp_select_hours);
		mMinutePickerDescription = res
				.getString(R.string.mdtp_minute_picker_description);
		...
		mTimePicker.initialize(getActivity(), this, mCalendar.get(Calendar.HOUR_OF_DAY),
				mCalendar.get(Calendar.MINUTE), mIs24HourMode);
		...
		Button mOtherInfo = (Button) view.findViewById(R.id.other_info);
		mOtherInfo.setText(TimeFormateTool.dateToString(mCalendar.getTime(), TimeFormateTool.FORMAT_DATE));
		mOtherInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				tryVibrate();
				if (mCallback != null) {
					mCallback.onDateClick(mTimePicker, mCalendar.getTime());
				}
				dismiss();
			}
		});
		...
		mOkButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (mCallback != null) {
					mCallback.onTimeSet(mTimePicker, mCalendar.getTime());
				}
				dismiss();
			}
		});
		mOkButton.setOnKeyListener(keyboardListener);
		mOkButton.setTypeface(TypefaceHelper.get(getDialog().getContext(),
				"Roboto-Medium"));
		...
		mCancelButton.setTypeface(TypefaceHelper.get(getDialog().getContext(),
				"Roboto-Medium"));
		...
		if (mIs24HourMode) {
			...
			updateAmPmDisplay(mCalendar.get(Calendar.HOUR_OF_DAY) < 12 ? AM : PM);
...
//		setMinute(mInitialMinute);
		...
		TextView timePickerHeader = (TextView) view
				.findViewById(R.id.time_picker_header);
		if (!mTitle.isEmpty()) {
		...
		ColorStateList doneTextColor = res
				.getColorStateList(R.color.mdtp_done_text_color);
		...
		ColorStateList darkDoneTextColor = res
				.getColorStateList(R.color.mdtp_done_text_color_dark);
		int darkDoneBackground = R.drawable.mdtp_done_background_color_dark;

		// Set the colors for each view based on the theme.
		// view.findViewById(R.id.time_display_background).setBackgroundColor(mThemeDark?
		// darkGray : accent);
		// view.findViewById(R.id.time_display).setBackgroundColor(mThemeDark?
		// darkGray : white);
		// ((TextView)
		// view.findViewById(R.id.separator)).setTextColor(mThemeDark? white :
		// timeDisplay);
		// ((TextView)
		// view.findViewById(R.id.ampm_label)).setTextColor(mThemeDark? white :
		// timeDisplay);
		// view.findViewById(R.id.line).setBackgroundColor(mThemeDark? darkLine
		// : line);
		// mOkButton.setTextColor(mThemeDark? darkDoneTextColor :
		// doneTextColor);
		mTimePicker.setBackgroundColor(mThemeDark ? lightGray
				: circleBackground);
		view.findViewById(R.id.time_picker_dialog).setBackgroundColor(
				mThemeDark ? darkBackgroundColor : backgroundColor);
		// mOkButton.setBackgroundResource(mThemeDark? darkDoneBackground :
		// doneBackground);
		
		setHour(mCalendar.get(Calendar.HOUR_OF_DAY),true);
	...
	private void updateAmPmDisplay(int amOrPm) {
		...
		mCalendar.set(Calendar.AM_PM, amOrPm==PM?Calendar.PM:Calendar.AM);
	}
	...
	private void setHour(int value, boolean announce) {
		...
		if (mIs24HourMode) {
			...
			mCalendar.set(Calendar.HOUR, value);
		}
