package com.accumulation.lib.ui.indicator;
 ...
 * 
 */
public class SlidingIconPageIndicator extends LinearLayout {
	/**
	 * 绘制三角形的画笔
	 */
	...
	public SlidingIconPageIndicator(Context context) {
		this(context, null);
	}

	public SlidingIconPageIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//				COUNT_DEFAULT_TAB);
	 ...
	 * @param datas
	 */
	public void setTabItemTitles(List<Integer> datas) {
		// 如果传入的list有值，则移除布局文件中设置的view
		if (datas != null && datas.size() > 0) {
			this.removeAllViews();
			this.mTabTitles = datas;

			for (Integer title : mTabTitles) {
				// 添加view
				addView(generateImageView(title));
			}
			// 设置item的click事件
			setItemClickEvent();
		}
	 ...
	 * @return
	 */
	private ImageView generateImageView(int res) {
	...
	public int getScreenWidth() {
		...
		return (int) (outMetrics.widthPixels*0.42f);
	}
