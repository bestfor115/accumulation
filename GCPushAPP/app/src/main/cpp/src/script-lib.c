...
int loadALLAPI(lua_State* L, const char * path) ;
static JNIEnv* attach_java_thread(const char* threadName) {
	...
	args.group = NULL;
	if ((result = (g_vm)->AttachCurrentThread(&e, (void*) &args))
		!= JNI_OK) {
...
static jboolean throw_runtime_exception(JNIEnv *env, const char*msg) {
	jclass clz = NULL;
	clz = (env)->FindClass("java/lang/RuntimeException");
	if (clz != NULL) {
...
static void native_exec(JNIEnv *env, jobject thiz, jstring jpath) {
	LOGD("native_exec ...");
	const char *s = NULL;
	s = env->GetStringUTFChars(jpath, NULL);
	loadALLAPI(L, s);
	env->ReleaseStringUTFChars(jpath, s);
}

static const char* const m_class_name =
		"com/xyz/gc/sdk/android/script/MScriptProtocol";
...
int api_openAPP(lua_State* L) {
	...
	if (env) {
		jstring js = env->NewStringUTF(param);
		env->CallStaticVoidMethod(m_player.clazz, m_player.api_openApp,
									 js);
...
int api_toast(lua_State* L) {
	...
	if (env) {
		jstring js = env->NewStringUTF(param);
		env->CallStaticVoidMethod(m_player.clazz, m_player.api_toast,
									 js);
		//env->ReleaseStringUTFChars(js, param);
	}
	LOGD("toast----");
...
int api_sleep(lua_State* L) {
	...
	JNIEnv *env = attach_java_thread("mplayer-callback");
	if (env) {
		env->CallStaticVoidMethod(m_player.clazz, m_player.api_sleep,
									 param);
	}
	return 0;
}
int api_sendMessage(lua_State* L) {
	...
	JNIEnv *env = attach_java_thread("mplayer-callback");
	if (env) {
		jstring js1 = env->NewStringUTF(param1);
		jstring js2 = env->NewStringUTF(param2);

		env->CallStaticVoidMethod(m_player.clazz,
									 m_player.api_sendMessage, js1, js2);
		env->ReleaseStringUTFChars(js1, param1);
		env->ReleaseStringUTFChars(js2, param2);
	}
	return 0;
}
