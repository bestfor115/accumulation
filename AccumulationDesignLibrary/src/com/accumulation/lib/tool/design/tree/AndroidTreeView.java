package com.accumulation.lib.tool.design.tree;
...
import com.accumulation.lib.tool.design.tree.holder.SimpleViewHolder;
import com.accumulation.lib.tool.design.tree.model.TreeNode;
...
public class AndroidTreeView {
	...
	public void setDefaultViewHolder(
			Class<? extends TreeNode.BaseNodeViewHolder> viewHolder) {
		defaultViewHolderClass = viewHolder;
	}

	public void setDefaultNodeClickListener(
	...
	public View getView(int style) {
		...
		if (style > 0) {
			ContextThemeWrapper newContext = new ContextThemeWrapper(mContext,
					style);
			view = use2dScroll ? new TwoDScrollView(newContext)
					: new ScrollView(newContext);
		} else {
			view = use2dScroll ? new TwoDScrollView(mContext) : new ScrollView(
					mContext);
		}
	...
	public void restoreState(String saveState) {
		if (!TextUtils.isEmpty(saveState)) {
			collapseAll();
			final String[] openNodesArray = saveState
					.split(NODES_PATH_SEPARATOR);
			final Set<String> openNodes = new HashSet<String>(
					Arrays.asList(openNodesArray));
	...
	public void caculateChoose(List list) {
		caculateChoose(list,mRoot);
	}
	public void caculateChoose(List list, TreeNode node) {
		List<TreeNode> nodes = node.getChildren();
		int N = nodes == null ? 0 : nodes.size();
		for (int i = 0; i < N; i++) {
			Object value = nodes.get(i).getValue();
			if (list.contains(value)) {
				selectNode(nodes.get(i), true);
			}
			caculateChoose(list,nodes.get(i));
		}
	...
	private void selectNode(TreeNode parent, boolean selected,
	...
	private TreeNode.BaseNodeViewHolder getViewHolderForNode(TreeNode node) {
		...
		if (viewHolder == null) {
			try {
				final Object object = defaultViewHolderClass.getConstructor(
						Context.class).newInstance(new Object[] { mContext });
