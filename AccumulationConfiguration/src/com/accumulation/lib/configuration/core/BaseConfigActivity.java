...
import com.accumulation.lib.configuration.core.ConfigState.GlobalFocusFrameListener;
import com.accumulation.lib.configuration.core.ExtJarLoader.ExtJarLoadListener;
import com.accumulation.lib.tool.debug.Logger;
import com.accumulation.lib.tool.net.cache.JSONApiHelper;
import com.accumulation.lib.tool.net.cache.JSONApiHelper.CallbackType;
import com.accumulation.lib.tool.net.cache.JSONApiHelper.StringResponseListener;
import com.accumulation.lib.tool.net.imgcache.ImageFetcher;
import com.accumulation.lib.tool.net.imgcache.SharedImageFetcher;
import com.accumulation.lib.ui.frameindicator.IFrameIndicator;
import com.accumulation.lib.ui.frameindicator.ViewFrameIndicator;
...
//			return;
//		}
		Logger.d("loadConfig start to callJSONAPI  url = " + url);
		JSONApiHelper.callJSONAPI(this, CallbackType.ForceUpdate, furl, null,
	...
	protected void parseConfig(String config, String url)
		...
		InterpreterManager.createManager(conf).work();
	...
	protected synchronized void showContent(Configuration conf) {
		...
		if (mFocusFrame != null) {
			...
			Logger.printAllChildren("[BEFORE CONF]", vg, false);
			mFocusFrame = null;
		}
		setContentView(root);
		root.requestFocus();
		...
		Logger.printAllChildren("[AFTER CONF]", vg, false);
		...
		if (mCurrentConfigUrl.startsWith("http://") && !isConnected()
				&& shouldLoadFallbackConfig(mCurrentConfigUrl)) {
			Logger.d("No connection, check if current cache is valid in 10 seconds");
			uiHandler.sendEmptyMessageDelayed(MSG_LOAD_FALLBACK_CONFIG, 10000);
		} else {
