...
public class ReflectingFrameLayout extends FrameLayout {
	...
	public ReflectingFrameLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	public ReflectingFrameLayout(Context context, View data) {
		super(context, data);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (reflectionHeight > 0 && width > 0) {
			if (sourceBitmap == null || sourceBitmap.getWidth() != getMeasuredWidth()
					|| sourceBitmap.getHeight() != reflectionHeight) {
				sourceBitmap = Bitmap
						.createBitmap(width, reflectionHeight, Bitmap.Config.ARGB_8888);
				tempCanvas = new Canvas(sourceBitmap);

				LinearGradient gradient = new LinearGradient(0, 0, 0, reflectionHeight,
						mAlphaStart, mAlphaEnd, TileMode.MIRROR);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (reflectionHeight > 0 && tempCanvas != null) {
			...
			int count = tempCanvas.saveLayer(0, 0, canvas.getWidth(), canvas.getHeight(), null,
			...
			canvas.drawBitmap(sourceBitmap, 0, childBottom, null);
		}
	...
	private int getReflectionHeight() {
		return (int) Math.min(getMeasuredHeight() - getMaxChildBottom(), getTotalChildHeight()
