...
import com.accumulation.lib.configuration.base.IConfigRebind;
...
public class EditTxt extends EditText implements IConfigView, IConfigRebind{
	...
	private void bindProperty(View data) {
		...
		bind = data.getBindByName(PROP_HINT);
		if(bind != null){
			this.setHint(bind.getValue().getvalue());
		}
		bind = data.getBindByName(PROP_HINT_COLOR);
		if(bind != null){
			Value v = bind.getValue();

			if(Value.TYPE_STRING.equals(v.getType())){
				setHintTextColor(PropertyUtils.parseColor(v.getvalue()));
			}
		}
